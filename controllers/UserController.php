...
class UserController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        $providerUserVisitLog = new \yii\data\ArrayDataProvider([
            'allModels' => $model->userVisitLogs,
        ]);
        return $this->render('view', [
            'model' => $this->findModel($id),
            'providerAuthAssignment' => $providerAuthAssignment,
            'providerUserVisitLog' => $providerUserVisitLog,
        ]);
    }
    ...
    * @return mixed
    */
    public function actionAddUserVisitLog()
    {
        if (Yii::$app->request->isAjax) {
            $row = Yii::$app->request->post('UserVisitLog');
            if (!empty($row)) {
                $row = array_values($row);
            }
            if((Yii::$app->request->post('isNewRecord') && Yii::$app->request->post('_action') == 'load' && empty($row)) || Yii::$app->request->post('_action') == 'add')
                $row[] = [];
            return $this->renderAjax('_formUserVisitLog', ['row' => $row]);
        } else {
            throw new NotFoundHttpException('The requested page does not exist.');
        }
    }
